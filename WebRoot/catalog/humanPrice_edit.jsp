...
<script type="text/javascript">
parent.window.document.getElementById('msgDiv').innerHTML = "编辑人员基价";
</script>
<title>修改基价</title>
</head>
	...
	<form action="" method="post" name="frm">
		...
		<td>
			<select class="select_01" name="cataId">
				...
				<option value="${cata.cataId}" <c:if test="${cata.cataId == priceHuman.catalog.cataId }">selected</c:if> >${cata.cataName}</option>
				</c:forEach>
			</select>
			<select class="select_01" name="deptRoleId" onchange="initDeptRolePrice()">
				...
				<option <c:if test="${deptRole.deptRoleId == priceHuman.deptRole.deptRoleId}">selected</c:if> id="deptRolePrice@${deptRole.deptRoleId}" value_price="${deptRole.deptRolePrice }" value="${deptRole.deptRoleId}"  >${deptRole.deptRoleName}</option>
				</c:forEach>
			</select>
		</td>
		</tr>
		...
		<select name="priceType" class="select_01">
			<option value="1" <c:if test="${priceHuman.priceType == '1' }">selected</c:if>>固定值</option>
			<option value="2" <c:if test="${priceHuman.priceType == '2' }">selected</c:if>>百分比</option>
		</select>
		</td>
		</tr>
		...
		<td><input type="text" class="input_02" name="price" value="${priceHuman.price }" /><input type="hidden" value="${priceHuman.priceId }" name="priceId" >
		元
		</td>
		</tr>
		...
		<td><textarea class="textarea_01" name="info" >${priceHuman.info }</textarea></td>
		</tr>
