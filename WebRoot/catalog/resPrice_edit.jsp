...
<script type="text/javascript">
parent.window.document.getElementById('msgDiv').innerHTML = "编辑资源基价";
</script>
<title>修改基价</title>
</head>
	...
	<form action="" method="post" name="frm">
		...
		<td>
			<select class="select_01" name="cataId">
				...
				<option value="${cata.cataId}" <c:if test="${cata.cataId == priceRes.catalog.cataId }">selected</c:if> >${cata.cataName}</option>
				</c:forEach>
			</select>
		</td>
		</tr>
		...
		<td><input type="text" class="input_01" name="resName" value="${priceRes.resName }" />
		</td>
		</tr>
		...
		<td><input type="text" class="input_01" name="resBrand" value="${priceRes.resBrand }" />
		</td>
		</tr>
		...
		<td><input type="text" class="input_01" name="provider" value="${priceRes.provider }" />
		</td>
		</tr>
		...
		<td><input type="text" class="input_02" name="resUnit" value="${priceRes.resUnit }" />
		</td>
		</tr>
		...
		<select name="priceType" class="select_01">
			<option value="1" <c:if test="${priceRes.priceType == '1' }">selected</c:if>>固定值</option>
			<option value="2" <c:if test="${priceRes.priceType == '2' }">selected</c:if>>百分比</option>
		</select>
		</td>
		</tr>
		...
		<td><input type="text" class="input_02" name="price" value="${priceRes.price }" /><input type="hidden" value="${priceRes.priceId }" name="priceId" >
		元
		</td>
		</tr>
		...
		<td><textarea class="textarea_01" name="info" >${priceRes.info }</textarea></td>
		</tr>
