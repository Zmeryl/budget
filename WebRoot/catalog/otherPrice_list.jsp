...
function kindsSearch(){
	...
	document.all.frm.action = "otherPriceAction.jhtml";
...
function doEdit(){
	...
	document.all.frm.target = "_self";
	document.all.frm.action = "otherPriceAction!showEditPriceOtherJSP.jhtml";
	document.all.frm.submit();
}
...
function doDelete(){
	...
	if(confirm("确定删除？")==true){
		...
		document.all.frm.target = "_self";
		document.all.frm.action = "otherPriceAction!deletePriceOther.jhtml";
		document.all.frm.submit();
	}
}
</script>
<script type="text/javascript">
parent.window.document.getElementById('msgDiv').innerHTML = "其他资源列表";
</script>
<title>人员基价</title>
</head>
<body>
<div class="tabcont">
		...
		<td>
			<a href="<%=strBase%>/catalog/otherPriceAction!showAddPriceOtherJSP.jhtml"><img src="<%=strBase%>/images/caozuo_01.gif" />添加</a>&nbsp;&nbsp;&nbsp;&nbsp;
		...
		<c:forEach items="${page.result}" var="priceOther" varStatus="priceResStatus" >
			<tr class="white">
				<td><input type="checkbox" value="${priceOther.priceId}" id="_IDCheck" name="_IDCheck"></td>
				<td><a href="otherPriceAction!viewPriceOther.jhtml?priceId=${priceOther.priceId}">${priceOther.cataName}&nbsp;</a></td>
				<td>${priceOther.resName}&nbsp;</td>
				<td>${priceOther.resUnit}&nbsp;</td>
				<td>
				<c:if test="${priceOther.priceType == '1'}">固定值</c:if>
				<c:if test="${priceOther.priceType == '2'}">百分比</c:if>
				&nbsp;</td>
				<td>${priceOther.price}&nbsp;</td>
				<td>${priceOther.publicRate}&nbsp;</td>
    	...
    	<tr class="titletop">
    		<td>
        	<ambow:pagination actionName="otherPriceAction" 
