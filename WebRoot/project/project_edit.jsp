...
function doEdit(){
	...
	document.all.frm.action = "projectAction!updateProject.jhtml";
...
function checkfrm(){
	...
	if(document.all.frm.managerName.value == ""){
		alert("请选择项目负责人");
		return false;
	}
...
<script type="text/javascript">
parent.window.document.getElementById('msgDiv').innerHTML = "编辑项目信息";
</script>
<title>修改项目属性</title>
</head>
...
<div class="tabcont">
	<form action="" method="post" name="frm">
		...
		<td>
			<select class="select_01" name="deptId">
				...
				<c:if test="${dept.superId == '0'}">
					<option value="${dept.deptId}" <c:if test="${dept.deptId eq prj.dept.deptId}">selected</c:if> >${dept.deptName}</option>
				</c:if>
				</c:forEach>
			</select>
		</td>
		...
		<td><input type="text" class="input_01" name="prjName" value="${prj.prjName}" />
		</td>
		</tr>
		...
		<td><input type="text" class="input_01" name="managerName" value="${prj.managerName }" readonly="readonly" onclick="showManagerAddJSP()" />
		<input type="hidden" name="managerId" value="${prj.manager.userId }"  />
		...
		<td><textarea class="textarea_01" name="prjDesc" >${prj.prjDesc }</textarea></td>
		</tr>
